...
then
  ...
  # Fix up problems with rms_obs_id, where Django deletes the _id on the end for some reason
  sed -e 's/rms_obs =/rms_obs_id =/' < /tmp/_models.py > /tmp/__models.py
  sed -e "s/rms_obs_id = models.ForeignKey(ObsGeneral, models.DO_NOTHING)/rms_obs_id = models.ForeignKey(ObsGeneral, models.DO_NOTHING, related_name='%(class)s_rms_obs_id')/g" < /tmp/__models.py > /tmp/_models.py

...
* **The `__sessionid` override.** It is how a test drives two carts in one process
  (the cross-session cart tests in `integration_tests/apps_db_tests` depend on it),
  and it is read ahead of Django's own session, so a change of precedence would
  silently put those tests back on one session.

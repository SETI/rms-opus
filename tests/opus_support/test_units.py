...
@pytest.mark.parametrize('converter',
                         [convert_to_default_unit, convert_from_default_unit])
def test_convert_passes_through_none_value(
        converter: Callable[..., Any]) -> None:
...
@pytest.mark.parametrize('converter',
                         [convert_to_default_unit, convert_from_default_unit])
def test_convert_passes_through_without_units(
        converter: Callable[..., Any]) -> None:
    """With no unit_id and no unit there is nothing to convert."""
    assert converter(10, None, None) == 10


@pytest.mark.parametrize(('converter', 'unit', 'expected'), [
...
def test_get_valid_units() -> None:
    ...
    assert get_valid_units('duration') == ['seconds', 'microseconds',
...
def test_get_unit_display_names() -> None:
    ...
    assert get_unit_display_names('datetime') == {'ymdhms': 'YMDhms',
                                                  'ydhms': 'YDhms',
                                                  'jd': 'JD',
                                                  'jed': 'JED',
                                                  'mjd': 'MJD',
...
@pytest.mark.parametrize('checker', [display_search_unit, display_result_unit,
                                     display_unit_ever])
@pytest.mark.parametrize(('unit_id', 'expected'), [
    (None, False),
    ('longitude', True),
...
def test_display_unit(checker: Callable[[str | None], bool],
                      unit_id: str | None, expected: bool) -> None:
...
def test_get_disp_default_and_avail_units_hidden() -> None:
    ...
    assert get_disp_default_and_avail_units('%d:range_cassini_rev_no') == (
        None, None, None)
...
@pytest.mark.parametrize(('numerical_format', 'unit_id', 'unit', 'expected'), [
...
def test_adjust_format_string_for_units(numerical_format: str,
                                        unit_id: str | None, unit: str | None,
                                        expected: str) -> None:
    """Decimal places grow or shrink with the size ratio to the default unit."""
    assert adjust_format_string_for_units(numerical_format, unit_id,
                                          unit) == expected
...
def test_format_unit_value_uses_unit_format_function(unit: str,
                                                     expected: str) -> None:
    """A unit that declares a format function delegates to it."""
    assert format_unit_value(600000000, None, 'datetime', unit) == expected


@pytest.mark.parametrize(('val', 'numerical_format', 'unit_id', 'unit',
                          'kwargs', 'expected'), [
    ...
    (100.1, '.3f', 'duration', 'seconds', {'keep_trailing_zeros': True},
     '100.100'),
    (100.1, '.3f', 'duration', 'days', {'keep_trailing_zeros': True},
     '0.00115856'),
    (100.1, '.3f', 'duration', 'days',
...
def test_format_unit_value_numeric(val: float, numerical_format: str | None,
                                   unit_id: str | None, unit: str | None,
                                   kwargs: dict[str, bool],
                                   expected: str) -> None:
    """Units with no format function are rendered with the numerical format."""
    assert format_unit_value(val, numerical_format, unit_id, unit,
                             **kwargs) == expected
...
def test_parse_form_type(form_type: str | None,
...
@pytest.mark.parametrize('getter',
                         [get_single_parse_function, get_single_format_function])
@pytest.mark.parametrize('unit_id', [None, 'datetime'])
def test_get_single_function_none(getter: Callable[..., Any],
...
def test_get_single_function(getter: Callable[..., Any],
                             expected: Callable[..., Any]) -> None:

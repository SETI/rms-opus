...
def test_parse_angle(parser: Callable[..., float], letter: str, text: str, expected: float) -> None:
...
def test_parse_angle_is_case_insensitive(
    parser: Callable[..., float], text: str, expected: float
) -> None:
...
def test_parse_angle_conversion_factor(
    parser: Callable[..., float], letter: str, text: str, expected: float
) -> None:
    """``conversion_factor`` applies only when the unit is stated explicitly."""
    assert parser(text.format(L=letter), conversion_factor=2) == expected * ANGLE_SCALE[letter]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param('0{L} ' + OVERFLOWING_DIGITS + 'm 0s', id='overflowing-minutes-component'),
...
def test_parse_angle_rejects_bad_components(
    parser: Callable[..., float], letter: str, text: str
) -> None:
...
def test_parse_angle_rejects_non_numeric(parser: Callable[..., float], text: str) -> None:
...
def test_parse_angle_rejects_malformed_triple(parser: Callable[..., float], text: str) -> None:
...
def test_parse_dms_hms_rejects_overflow(parser: Callable[..., float], text: str) -> None:
...
@pytest.mark.parametrize(
    ('val', 'unit', 'numerical_format', 'keep_zeros', 'expected'),
    [
        # Plain degrees.
...
def test_format_dms_hms(
    val: float, unit: str, numerical_format: str, keep_zeros: bool, expected: str
) -> None:
    """Angles format as plain numbers or as DMS/HMS depending on the unit."""
    assert (
        format_dms_hms(
            val, unit=unit, numerical_format=numerical_format, keep_trailing_zeros=keep_zeros
        )
        == expected
    )

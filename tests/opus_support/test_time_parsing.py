...
@pytest.mark.parametrize(('formatter', 'tai', 'expected'), [
...
def test_format_time(formatter: Callable[[float], str], tai: float,
                     expected: str) -> None:
...
@pytest.mark.parametrize(('iso', 'unit', 'expected'), [
...
def test_parse_time_epochs(iso: str, unit: str | None,
                           expected: float) -> None:
    """Prefixed and bare epoch values parse to the expected TAI value."""
    assert parse_time(iso, unit=unit) == pytest.approx(expected,
                                                       abs=TIME_ABS_TOL)


@pytest.mark.parametrize(('iso', 'message'), [
...
def test_parse_time_round_trip(formatter: Callable[[float], str], iso: str,
                               expected: str) -> None:

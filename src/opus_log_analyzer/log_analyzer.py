...
def _create_argument_parser() -> argparse.ArgumentParser:
    ...
    group2.add_argument(
        '--by-ip', action='store_true', dest='by_ip', help='Sorts batched logs by host ip'
    )
    group2.add_argument(
        '--by-time',
        action='store_false',
    ...
    parser.add_argument(
        '--xxdns-cache', action='store_true', dest='dns_cache', help=argparse.SUPPRESS
    )

    # Debugging hack that shows all log entries
    parser.add_argument(
        '--xxshowall', action='store_true', dest='debug_show_all', help=argparse.SUPPRESS
    )
...
def main(arguments: list[str] | None = None) -> None:
    ...
    args.ip_to_host_converter = IpToHostConverter.get_ip_to_host_converter(**vars(args))

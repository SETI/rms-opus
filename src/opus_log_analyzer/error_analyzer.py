...
class ErrorReader:
    ...
    def run(self) -> None:
        ...
        log_entries_by_host_ip = {
            ...
            for host_ip, log_entries in itertools.groupby(log_entries, attrgetter('host_ip'))
        }
    ...
    def _check_one_ip(self, error_entries: list[ErrorEntry], log_entries: list[LogEntry]) -> None:
        ...
        while error_entries_deque:
            ...
            while error_entries_deque and merge_error_entries(
                these_error_entries, error_entries_deque[0]
            ):
    ...
    def _show_results(self) -> None:
        ...
        for key in sorted(
            seen_errors.keys(), key=lambda strs: (-len(seen_errors[strs]), -len(strs), strs)
        ):
            sorted_error_and_log_pairs = sorted(
                seen_errors[key], key=lambda x: x.error_entries[0].time
            )
            min_time = sorted_error_and_log_pairs[0].error_entries[0].time
            all_sorted_log_entries = [
                sorted(log_entries, key=attrgetter('time'))
                for _, log_entries in sorted_error_and_log_pairs
            ]
    ...
    def __generate_text_output(
        self, results: list[tuple[tuple[str, ...], int, datetime.datetime, list[list[LogEntry]]]]
    ) -> None:
        ...
        for key, count, min_time, all_sorted_log_entries in results:
            ...
            if count == 1:
                ...
                output.write(
                    f'This error occurs {count} times; the first occurrence is at {min_time}.\n'
                )
            for line in key:
                output.write(f'{line}\n')
...
def main(arguments: list[str] | None = None) -> None:
    ...
    parser.add_argument('--cronjob', action='store_true', dest='cronjob', help='DEPRECATED')
    ...
    parser.add_argument(
        '--ignore-errors-file', type=argparse.FileType('r'), default=None, dest='ignore_errors_file'
    )

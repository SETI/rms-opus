...
_CASSINI_PHASE_NAME_MAPPING = (
    ...
    (
        'Venus 1 Encounter',
        cached_tai_from_iso('1998-116T00:00:00.000'),
        cached_tai_from_iso('1998-117T00:00:00.000'),
    ),
    (
        'Venus 2 Encounter',
        cached_tai_from_iso('1999-175T00:00:00.000'),
        cached_tai_from_iso('1999-176T00:00:00.000'),
    ),
    (
        'Earth Encounter',
        cached_tai_from_iso('1999-230T00:00:00.000'),
        cached_tai_from_iso('1999-231T00:00:00.000'),
    ),
    (
        'Jupiter Encounter',
        cached_tai_from_iso('2000-365T00:00:00.000'),
...
class ObsCassiniCommon(ObsBase):
    ...
    def _parse_cassini_sclk(
        self, sclk: str, log_func: Callable[[str], None] | None = None
    ) -> FloatField:
        ...
        """
        return self._parse_sclk(opus_support.parse_cassini_sclk, sclk, 'Cassini', log_func)
    ...
    def _cassini_valid_obs_name(self, obs_name: str | None) -> TypeGuard[str]:
        ...
        # Try without _INST
        ret = re.fullmatch(
            r'([A-Z]{2,5}|22NAV)_([0-2]\d\d|00[A-C]|C\d\d)[A-Z]{2}_[0-9A-Z]+\d\d\d', obs_name
        )
        return bool(ret)
    ...
    def _coiss_wavelength_helper(
        self, camera: str | None, filter1: str | None, filter2: str | None
        ...
        self._log_nonrepeating_warning(
            'Ignoring unknown COISS filter combination ' + f'{key[0]}/{key[1]}/{key[2]}'
        )
        return None, None, None

    def _combined_filter(
        self, camera: str | None = None, filter1: str | None = None, filter2: str | None = None
    ) -> str:
        ...
        if filter1 == 'CL1' and filter2 == 'CL2':
            ...
            if filter1 and 'P' in filter1:
                ...
                if (same_or_unknown_wl and filter1 > filter2) or (
                    not same_or_unknown_wl and wl1 is not None and wl2 is not None and wl1 > wl2
                ):
    ...
    def field_obs_mission_cassini_rev_no_int(self) -> IntField:
        ...
        try:
            ...
            self._log_nonrepeating_error(f'Unable to parse Cassini orbit "{rev_no}": {e}')
            return None
        return rev_no_cvt
    ...
    def field_obs_mission_cassini_cassini_target_code(self) -> MultFieldRet:
        ...
        if target_code in _CASSINI_TARGET_CODE_MAPPING:
            return self._create_mult(
                col_val=target_code, disp_name=_CASSINI_TARGET_CODE_MAPPING[target_code]
            )

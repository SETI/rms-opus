...
def api_normalize_input(request: HttpRequest) -> HttpResponse:
    ...
    (selections, _extras) = url_to_search_params(
        request.GET, allow_errors=True, return_slugs=True, pretty_results=True
    )
    if selections is None:
        log.error(
            'api_normalize_input: Could not find selections for' + ' request %r', str(request.GET)
        )
...
@api_view
def api_string_search_choices(request: HttpRequest, slug: str, *, api_code: int) -> HttpResponse:
    ...
    """
    if not request or request.GET is None or request.META is None:
        raise Http404(http404_no_request(f'/__api/stringsearchchoices/{slug}.json'))
    ...
    if not param_info:
        log.error('api_string_search_choices: unknown slug "%r"', slug)
    ...
    (selections, extras) = url_to_search_params(request.GET, allow_regex_errors=True)
    if selections is None:
        log.error(
            'api_string_search_choices: Could not find selections for' + ' request %r',
            str(request.GET),
        )
        raise Http400Error(http400_search_params_invalid(request))
    ...
    like_query, like_params = get_string_query(selections, param_qualified_name, query_qtype_list)
    if like_query is None:
        # This is usually caused by a bad regex
        result = {'choices': [], 'full_search': True, 'truncated_results': False}
        result['reqno'] = reqno
    ...
    if not user_query_table:  # pragma: no cover - internal or database failure
        log.error(
            'api_string_search_choices: get_user_query_table failed '
    ...
    try:
        ...
        log.error('api_string_search_choices: Bad limit for' + ' request %r', str(request.GET))
    ...
    partial_query_hash = hashlib.md5(str.encode(partial_query), usedforsecurity=False).hexdigest()
    ...
    if not do_simple_search:
        ...
        select.add_from(param_category).add_join(
            'INNER', user_query_table, search_cache_join_condition(param_category, user_query_table)
        )
    ...
    if final_results is None:  # pragma: no cover - can't trigger during testing
        ...
        if partial_query:
            ...
            try:
                ...
                pattern = regex.compile(f'({esc_partial_query})', regex.IGNORECASE | regex.V1)
                final_results = [pattern.sub('<b>\\1</b>', x) for x in final_results]
            except Exception:
    ...
    **The shape of a selections value depends on the mode flags below**, which is
    ...
    for slug, value in search_params:
        ...
        if slug.startswith('qtype-'):  # like qtype-time=all
            ...
            if slug_no_num != slug:
                log.error(
                    'url_to_search_params: qtype slug has ' + 'numeric suffix "%r"', orig_slug
                )
                return None, None
            ...
            slug_no_num = strip_numeric_suffix(slug)
            if slug_no_num != slug:
                log.error('url_to_search_params: unit slug has ' + 'numeric suffix "%r"', orig_slug)
                return None, None
            param_info = get_param_info_by_slug(slug, 'qtype')
            ...
            if slug_no_num != slug:
                log.error(
                    'url_to_search_params: sourceunit slug has ' + 'numeric suffix "%r"', orig_slug
                )
                return None, None
            param_info = get_param_info_by_slug(slug, 'qtype')
        else:
            param_info = get_param_info_by_slug(slug, 'search')
        if not param_info:
            log.error('url_to_search_params: unknown slug "%r"', orig_slug)
            return None, None
        ...
        if qtype_slug in request_get:
            ...
            if valid_qtypes is None or qtype_val not in valid_qtypes:
                if allow_errors:  # pragma: no cover - protection against future bugs
                    ...
                    log.error(
                        'url_to_search_params: Bad qtype value for ' + '"%r": %r',
                        qtype_slug,
                        str(qtype_val),
                    )
                    return None, None
        ...
        if unit_slug in request_get:
            ...
            if valid_units is None or unit_val not in valid_units:
                if allow_errors:  # pragma: no cover - protection against future bugs
                    ...
                    log.error(
                        'url_to_search_params: Bad unit value for ' + '"%r": %r',
                        unit_slug,
                        str(unit_val),
                    )
                    return None, None
        ...
        if sourceunit_slug in request_get:
            ...
            if valid_units is None or sourceunit_val not in valid_units:
                log.error(
                    'url_to_search_params: Bad sourceunit value' + ' for "%r": %r',
                    sourceunit_slug,
                    str(sourceunit_val),
                )
        ...
        if form_type in settings.MULT_FORM_TYPES:
            ...
            if clause_num_str:
                log.error(
                    'url_to_search_params: Mult field "%r" has clause'
                    + ' number where none permitted',
                    orig_slug,
                )
                return None, None
            ...
            mult_values = [
                x['pk']
                for x in list(
                    model.objects.filter(Q(label__in=new_val) | Q(value__in=new_val)).values('pk')
                )
            ]
            ...
            if len(mult_values) != len(new_val):
                if allow_errors:
                    ...
                    log.error(
                        'url_to_search_params: Bad mult data for "%r", ' + 'wanted "%r" found "%r"',
                        param_qualified_name,
                        str(new_val),
                        str(mult_values),
                    )
        ...
        if form_type in settings.RANGE_FORM_TYPES:
                ...
                if new_slug in request_get:
                    ...
                    if value:
                        try:
                            ...
                            # unit conversion.
                            new_value = parse_unit_value(
                                value, form_type_format, form_type_unit_id, sourceunit_val
                            )
                            if sourceunit_val is not None:
                                default_val = convert_to_default_unit(
                                    new_value, form_type_unit_id, sourceunit_val
                                )
                                new_value = convert_from_default_unit(
                                    default_val, form_type_unit_id, unit_val
                                )
                            ...
                            default_val = convert_to_default_unit(
                                new_value, form_type_unit_id, unit_val
                            )
                            ...
                            if not allow_errors:
                                log.error(
                                    'url_to_search_params: Unit ID "%r" '
            ...
            if return_slugs:
                ...
                allow_empty or new_values[0] is not None or new_values[1] is not None
            ):  # pragma: no cover -
        ...
        if is_unit or unit_val is not None:  # pragma: no cover -
            ...
            log.error('url_to_search_params: String field "%r" has unit', orig_slug)
            return None, None
        new_value = ''
        ...
        if (
            new_value
            and qtype_val == 'regex'
            and not allow_regex_errors
            and not _valid_regex(new_value)
        ):
            if not allow_errors:
                log.error(
                    'url_to_search_params: String "%r" ' + 'slug "%r" is not a valid regex',
                    new_value,
                    slug,
                )
                return None, None
        ...
        elif allow_empty or (new_value is not None and new_value != ''):  # pragma: no cover -
            # This will always be True because the normalized
            # clause numbers will be in order, and thus it will
            # always be necessary to add a new entry.
...
def get_user_query_table(
    selections: dict[str, Any], extras: dict[str, Any], api_code: int | None = None
) -> str | None:
    ...
    if cache_table_num is None:  # pragma: no cover - database error
        log.error(
            'get_user_query_table: Failed to make entry in user_searches'
            + ' *** Selections %r *** Extras %r',
            str(selections),
            str(extras),
        )
    ...
    if sql is None:  # pragma: no cover - already caught by previous checks
        log.error(
            'get_user_query_table: construct_query_string failed'
            + ' *** Selections %r *** Extras %r',
            str(selections),
            str(extras),
        )
    ...
    if not sql:  # pragma: no cover - not possible
        log.error(
            'get_user_query_table: Query string is empty' + ' *** Selections %r *** Extras %r',
            str(selections),
            str(extras),
        )
        return None
    ...
    create_sql = sql_builder.create_table_from_select_sql(
        cache_table_name, sql, column_defs=sql_builder.CACHE_TABLE_COLUMN_DEFS
    )
    ...
    try:
        ...
        log.exception(
            'get_user_query_table: "%r" with params "%r" failed', create_sql, str(tuple(params))
        )
        return None
    log.debug(
        'API %r (%.3f) get_user_query_table: %r *** PARAMS %r',
        str(api_code),
        time.time() - time1,
        create_sql,
        str(params),
...
def set_user_search_number(selections: dict[str, Any], extras: dict[str, Any]) -> tuple[int, bool]:
    ...
    selections_hash = hashlib.md5(str.encode(selections_json), usedforsecurity=False).hexdigest()
    ...
    if 'qtypes' in extras:
        ...
        for qtype, val in qtypes.items():
            ...
            if (
                qtype_no_num in selections
                or qtype_no_num + '1' in selections
                or qtype_no_num + '2' in selections
            ):
                new_qtypes[qtype] = val
        if len(new_qtypes):
            qtypes_json = str(json.dumps(sort_dictionary(new_qtypes)))
            qtypes_hash = hashlib.md5(str.encode(qtypes_json), usedforsecurity=False).hexdigest()
    ...
    if 'units' in extras:
        ...
        for unit, val in units.items():
            ...
            if (
                unit_no_num in selections
                or unit_no_num + '1' in selections
                or unit_no_num + '2' in selections
            ):
                new_units[unit] = val
        if len(new_units):
            units_json = str(json.dumps(sort_dictionary(new_units)))
            units_hash = hashlib.md5(str.encode(units_json), usedforsecurity=False).hexdigest()
    ...
    if 'order' in extras and extras['order'][0]:
        ...
        order_hash = hashlib.md5(str.encode(order_json), usedforsecurity=False).hexdigest()

    cache_key = (
        settings.CACHE_SERVER_PREFIX
        + settings.CACHE_KEY_PREFIX
        + ':usersearchno:selections_hash:'
        + str(selections_hash)
        + ':qtypes_hash:'
        + str(qtypes_hash)
        + ':units_hash:'
    ...
    try:
        ...
        duplicates = UserSearches.objects.filter(
            selections_hash=selections_hash,
            qtypes_hash=qtypes_hash,
            units_hash=units_hash,
            order_hash=order_hash,
        ...
        s = duplicates[0]
        log.exception(
            'set_user_search_number: Multiple entries in user_searches'
...
def get_param_info_by_slug(
    slug: str, source: str, allow_units_override: bool = False, check_valid_units: bool = True
    ...
    if source == 'qtype' and slug[-1] in ('1', '2'):  # pragma: no cover -
        ...
        log.error(
            'get_param_info_by_slug: Qtype slug "%r" has unpermitted ' + 'numeric suffix', slug
        )
        return None
    ...
    if pi:
        if source == 'col' and allow_units_override:  # pragma: no cover -
            ...
            if (
                check_valid_units
                and desired_units is not None
                and not pi.is_valid_unit(desired_units)
            ):
                log.error(
                    'get_param_info_by_slug: Slug "%r" unit "%r" invalid -using default',
                    slug,
                    desired_units,
                )
                desired_units = None
        ...
        if source == 'search':
            ...
            (form_type, _form_type_format, _form_type_unit_id) = parse_form_type(pi.form_type)
    ...
    log.error('get_param_info_by_slug: Slug "%r" source "%r" not found', slug, source)
...
def construct_query_string(
    selections: dict[str, Any], extras: dict[str, Any]
    ...
    for param_qualified_name in sorted(selections.keys()):
        ...
        if not param_info:
            log.error(
                'construct_query_string: No param_info for "%r"'
                + ' *** Selections %r *** Extras *** %r',
                param_qualified_name,
                str(selections),
                str(extras),
        ...
        if form_type in settings.MULT_FORM_TYPES:
            ...
            mult_values = [
                x['pk']
                for x in list(
                    model.objects.filter(Q(label__in=value_list) | Q(value__in=value_list)).values(
                        'pk'
                    )
                )
            ]
            if len(mult_values) != len(value_list):
                log.error(
                    'construct_query_string: Bad mult data for "%r", '
                    + 'found %r'
                    + ' *** Selections %r *** Extras *** %r',
                    param_qualified_name,
                    str(mult_values),
            ...
            if form_type == 'LONG':
                ...
                clause, params = get_longitude_query(
                    selections, param_qualified_name, qtypes, units
                )
            else:
                # Get the range query object and append it to the query
                clause, params = get_range_query(selections, param_qualified_name, qtypes, units)
            ...
            clause, params = get_string_query(selections, param_qualified_name, qtypes)
            if clause is None:
                return None, None
            # These builders return both halves together or neither.
            assert params is not None
            ...
            log.error(
                'construct_query_string: Unknown field type "%r" for ' + 'param "%r"',
                form_type,
                param_qualified_name,
            )
            return None, None
    ...
    if 'order' in extras:
        ...
        (order_terms, order_mult_tables, order_obs_tables) = create_order_by_terms(
            order_params, descending_params
        )
...
def get_string_query(
    selections: dict[str, Any], param_qualified_name: str, qtypes: list[str]
    ...
    if len(qtypes) != len(values):
        log.error(
            'get_string_query: Inconsistent qtype/values lengths '
            + 'for "%r" '
    ...
    for idx in range(len(values)):
        ...
        if qtype == 'contains':
            clause = sql_builder.binary_op(
                param_column, 'LIKE', sql_builder.value('%' + value + '%')
            )
        elif qtype == 'begins':
            clause = sql_builder.binary_op(param_column, 'LIKE', sql_builder.value(value + '%'))
        elif qtype == 'ends':
            clause = sql_builder.binary_op(param_column, 'LIKE', sql_builder.value('%' + value))
        elif qtype == 'matches':
            clause = sql_builder.binary_op(param_column, '=', sql_builder.value(value))
        elif qtype == 'excludes':
            clause = sql_builder.binary_op(
                param_column, 'NOT LIKE', sql_builder.value('%' + value + '%')
            )
        elif qtype == 'regex':
            if not _valid_regex(value):
                return None, None
            clause = sql_builder.binary_op(param_column, 'RLIKE', sql_builder.value(value))
        else:  # pragma: no cover - protecting against future bugs
            log.error(
                'get_string_query: Unknown qtype %r '
                + 'for "%r" '
                + '*** Selections %r *** Qtypes %r ***',
                qtype,
                param_qualified_name,
...
def get_range_query(
    ...
    (_form_type, _form_type_format, form_type_unit_id) = parse_form_type(param_info.form_type)
    ...
    if (
        len(qtypes) != len(values_min)
        or len(units) != len(values_min)
        or len(values_min) != len(values_max)
    ):
        log.error(
            'get_range_query: Inconsistent qtype/unit/min/max lengths '
            + 'for "%r" '
            + '*** Selections %r *** Qtypes %r *** Units %r',
            param_qualified_name,
            str(selections),
    ...
    for idx in range(len(values_min)):
        unit = units[idx]
        try:
            value_min = convert_to_default_unit(values_min[idx], form_type_unit_id, unit)
            value_max = convert_to_default_unit(values_max[idx], form_type_unit_id, unit)
        except KeyError:
            log.error(
                'get_range_query: Unknown unit "%r" for "%r" '
                + '*** Selections %r *** Qtypes %r *** Units %r',
                unit,
                param_qualified_name,
                str(selections),
            ...
            log.error(
                'get_range_query: Unit "%r" on "%r" conversion failed '
                + '*** Selections %r *** Qtypes %r *** Units %r',
                unit,
                param_qualified_name,
                str(selections),
        ...
        if qtype == 'all':
            ...
            log.error(
                'get_range_query: Unknown qtype "%r" '
                + 'for "%r" '
                + '*** Selections %r *** Qtypes %r *** Units %r',
                qtype,
                param_qualified_name,
        ...
        side_clauses = [
            sql_builder.binary_op(side_column, operator, sql_builder.value(side_value))
...
def get_longitude_query(
    ...
    if (
        ...
        log.error(
            'get_longitude_query: Inconsistent qtype/unit/min/max lengths '
            + 'for "%r" '
            + '*** Selections %r *** Qtypes %r *** Units %r',
            param_qualified_name,
            str(selections),
    ...
    for idx in range(len(values_min)):
        ...
        try:
            ...
            log.error(
                'get_longitude_query: Unknown unit "%r" for "%r" '
                + '*** Selections %r *** Qtypes %r *** Units %r',
                unit,
                param_qualified_name,
                str(selections),
            ...
            log.error(
                'get_longitude_query: Unit "%r" on "%r" conversion '
                + 'failed '
                + '*** Selections %r *** Qtypes %r *** Units %r',
                unit,
                param_qualified_name,
        ...
        if value_min is None or value_max is None:
            ...
            new_selections = {
                param_qualified_name_min: [value_min],
                param_qualified_name_max: [value_max],
            }
            new_qtypes = [qtype]
            range_clause, range_params = get_range_query(
                new_selections, param_qualified_name, new_qtypes, None
            )
            ...
            if qtype != 'any':
                log.error(
                    'get_longitude_query: Bad qtype "%r" for "%r" '
                    + '*** Selections %r *** Qtypes %r *** Units %r',
                    qtype,
                    param_qualified_name,
                    str(selections),
            ...
            clause = sql_builder.join_exprs(
                [
                    sql_builder.binary_op(param_column, '>=', sql_builder.value(value_min)),
                    sql_builder.binary_op(param_column, '<=', sql_builder.value(value_max)),
                ],
                operator,
            )
            ...
            if value_max >= value_min:
                longit = (value_min + value_max) / 2.0
            else:
                longit = (value_min + value_max + 360.0) / 2.0
            longit = longit % 360.0
            d_long = (longit - value_min) % 360.0
            ...
            if qtype == 'any' or qtype == '':
                bound = sql_builder.binary_op(sql_builder.value(d_long), '+', column_d_long)
            elif qtype == 'all':
                bound = sql_builder.binary_op(column_d_long, '-', sql_builder.value(d_long))
            elif qtype == 'only':
                bound = sql_builder.binary_op(sql_builder.value(d_long), '-', column_d_long)
            else:
                log.error(
                    'get_longitude_query: Unknown qtype "%r" '
                    + 'for "%r"'
                    + '*** Selections %r *** Qtypes %r *** Units %r',
                    qtype,
                    param_qualified_name,
...
def add_obs_table_joins(from_source: sql_builder.FromSource, obs_tables: Iterable[str]) -> None:
    ...
    for table in obs_tables:
        ...
        from_source.add_join(
            ...
            sql_builder.columns_equal(
                sql_builder.column('id', 'obs_general'), sql_builder.column('obs_general_id', table)
            ),
        )


def add_mult_table_joins(
    from_source: sql_builder.FromSource, mult_tables: Iterable[tuple[str, bool, str, str]]
) -> None:
    ...
    for mult_table, is_multigroup, category, field_name in mult_tables:
        ...
        from_source.add_join(
            'LEFT',
            mult_table,
            sql_builder.columns_equal(mult_column, sql_builder.column('id', mult_table)),
        )


def search_cache_join_condition(table_name: str, cache_table_name: str) -> sql_builder.Expr:
    ...
    return sql_builder.columns_equal(own_column, sql_builder.column('id', cache_table_name))
...
def is_single_column_range(param_qualified_name: str) -> bool:
    ...
    name_no_num = strip_numeric_suffix(name)
    try:
        ParamInfo.objects.get(category_name=cat_name, name=name_no_num)
    except ParamInfo.DoesNotExist:
        return False
...
def parse_order_slug(all_order: str | None) -> tuple[list[str] | None, list[bool] | None]:
    ...
    for order in orders:
        ...
        if not param_info:
            log.error('parse_order_slug: Unable to resolve order ' + 'slug "%r"', order)
            return None, None
...
def create_order_by_terms(
    order_params: list[str], descending_params: list[bool]
) -> tuple[list[tuple[Any, bool]] | None, set[tuple[str, bool, str, str]] | None, set[str] | None]:
    ...
    for i in range(len(order_params)):
        ...
        if not pi:
            log.error('create_order_by_terms: Unable to resolve order' + ' slug "%r"', order_slug)
        ...
        if form_type in settings.MULT_FORM_TYPES:
            ...
            order_mult_tables.add((mult_table, is_multigroup, pi.category_name, pi.name))

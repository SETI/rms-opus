...
urlpatterns = [
    ...
    re_path(
        r'^__api/metadata/(?P<opus_id>[-\w]+).(?P<fmt>json|html|csv)$', api_get_metadata_internal
    ),
    # Backwards compatibility
    re_path(r'^api/metadata_v2/(?P<opus_id>[-\w]+).(?P<fmt>json|html|csv)$', api_get_metadata),
    re_path(
        r'^api/images/(?P<size>thumb|small|med|full).(?P<fmt>json|html|csv)$',
        api_get_images_by_size,
    ),
    re_path(r'^api/images.(?P<fmt>json|csv)$', api_get_images),
    re_path(
        r'^api/image/(?P<size>thumb|small|med|full)/(?P<opus_id>[-\w]+).(?P<fmt>json|html|csv)$',
        api_get_image,
    ),
    re_path(
        r'^__api/image/(?P<size>thumb|small|med|full)/(?P<opus_id>[-\w]+).(?P<fmt>json)$',
        api_get_image,
    ),

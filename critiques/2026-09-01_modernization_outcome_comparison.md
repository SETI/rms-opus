...
| 8 | Dependencies not declared in a `pyproject.toml`/`requirements.txt` (Medium) | **Fixed, with a live caveat** | `pyproject.toml` declares every dependency and justifies every floor. The caveat is a new failure of the same kind: `rms-pdsfile>=0.0.18` is not what CI installs — see §5. |

...
urlpatterns = [
    ...
    url(r'^api/image/(?P<size>[thumb|small|med|full]+)/(?P<opus_id>[0-9a-zA-Z\-_]+).(?P<fmt>[json|zip|html|csv]+)$', get_image),
    url(r'^api/files/(?P<opus_id>[0-9a-zA-Z\-_]+).(?P<fmt>[json|zip|html|csv]+)$',get_files_API),

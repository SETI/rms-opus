...
def get_all_in_collection(request):
    ...
    if not request.session.get('has_session'):
        ...
        sql = 'select rms_obs_id from ' + connection.ops.quote_name(coll_table_name)
        cursor.execute(sql)
        rms_obs_ids = [n[0] for n in cursor.fetchall()]
        return rms_obs_ids
...
def get_collection_table(session_id):
    ...
    try:
        sql = 'select rms_obs_id from ' + connection.ops.quote_name(coll_table_name) + ' limit 1'
        cursor.execute(sql)
...
def bulk_add_to_collection(rms_obs_id_list, session_id):
    cursor = connection.cursor()
    if type(rms_obs_id_list).__name__ == 'str':
        rms_obs_id_list = [rms_obs_id_list]
    coll_table_name = get_collection_table(session_id)
    placeholders = ['(%s)' for i in range(len(rms_obs_id_list))]
    values_str = ','.join(placeholders)
    sql = 'replace into ' + connection.ops.quote_name(coll_table_name) + ' (rms_obs_id) values %s' % values_str
    cursor.execute(sql, tuple(rms_obs_id_list))

def add_to_collection(rms_obs_id, session_id):
    cursor = connection.cursor()
    coll_table_name = get_collection_table(session_id)
    # first remove
    remove_from_collection(rms_obs_id, session_id)
    sql = 'replace into ' + connection.ops.quote_name(coll_table_name) + ' (rms_obs_id) values (%s)'
    cursor.execute(sql, (rms_obs_id,))

def remove_from_collection(rms_obs_id, session_id):
    cursor = connection.cursor()
    coll_table_name = get_collection_table(session_id)
    sql = 'delete from ' + connection.ops.quote_name(coll_table_name) + ' where rms_obs_id = %s'
    cursor.execute(sql, (rms_obs_id,))
...
def get_collection_in_page(page, session_id):
    ...
    for p in page:
        rms_obs_id = p[0]
        sql = 'select rms_obs_id from ' + connection.ops.quote_name(coll_table_name) + ' where rms_obs_id = %s'
        cursor.execute(sql, (rms_obs_id,))
        row = cursor.fetchone()
        if row is not None:
            collection_in_page.append(rms_obs_id)
...
def edit_collection(request, **kwargs):
    ...
    """
    edits a single rms_obs_id in a user collection (user "selections")
    # edits?
    """
    if not request.session.get('has_session'):
    ...
    if type(checkArgs).__name__ == 'list' and checkArgs:
        (action, rms_obs_id, request_no, expected_request_no) = checkArgs
    else:
        return HttpResponse(json.dumps({"err":checkArgs}))

    # just add this request to the queue, every request gets queued
    add_to_queue(request, request_no, action, rms_obs_id)
    ...
    if get_queued(request, expected_request_no):
        ...
        (request,action,rms_obs_id) = get_queued(request, expected_request_no)
    else:
...
def edit_collection_addall(request, **kwargs):
    ...
    sql = "replace into " + connection.ops.quote_name(coll_table_name) + \
          " (id, rms_obs_id) select o.id, o.rms_obs_id from obs_general o, " + connection.ops.quote_name(query_table_name) + \
          " s where o.id = s.id"
    cursor.execute(sql)
...
def edit_collection_range(request, **kwargs):
    update_metrics(request)
    """
    the request will come in not as a single rms_obs_id
    but as min and max rms_obs_ids + a range in a search query
    ...
    session_id = request.session.session_key
    colls_table_name = get_collection_table(session_id)
    (action, rms_obs_id, request_no, expected_request_no) = check_collection_args(request, **kwargs)
    ...
    rms_obs_id_key = column_slugs.split(',').index('ringobsid')

    # return HttpResponse(json.dumps(data['page']));
    for row in data['page']:

        rms_obs_id = row[rms_obs_id_key]

        if rms_obs_id == min_id:
            in_range = True;

        if in_range:
            selected_range.append(rms_obs_id)

        if in_range & (rms_obs_id == max_id):
...
def view_collection(request, collection_name, template="collections.html"):
    ...
    page_ids = [o['rms_obs_id'] for o in results]
...
def check_collection_args(request,**kwargs):
    ...
    return [action, rms_obs_id, request_no, expected_request_no]
...
def get_queued(request, request_no):
    """ get the next request in queue
        if get_queued(request, expected_request_no):
        (collection_name,action,rms_obs_id) = get_queued(request, expected_request_no)
    """
    queue = request.session.get("queue", {})
    try:

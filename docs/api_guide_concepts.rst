...
* **any**: The observation range overlaps at least some with the search range. In other words, the observation maximum is greater than or equal to the search minimum, **and** the observation minimum is less than or equal to the search maximum. Both conditions have to hold: either one on its own is satisfied by ranges that do not overlap at all. This option is used to request the widest possible set of observations that contain at least some of the range you are searching for. This is the default if no qtype is given.

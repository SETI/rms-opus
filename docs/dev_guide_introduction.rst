...
use. The database backend is written against an abstraction
(:mod:`opus_import.importdb`) that keeps room for another brand, but MySQL is the
only brand implemented.

var o_detail = {
    ...
    getDetail: function (rms_obs_id) {
        ...
        $('#detail_extra').html(rms_obs_id);

        $("#detail").load("/opus/api/detail/" + rms_obs_id + ".html", function(){

            // get the column metadata, this part is fast
            url = "/opus/api/metadata/" + rms_obs_id + ".html?" + o_hash.getHash();
            $("#cols_metadata")
                .load(url, function() {
            ...
            url = "/opus/api/categories/" + rms_obs_id + ".json?" + o_hash.getHash();
            $.getJSON(url, function(json) {
                for (var index in json) {
                  ...
                  $("#all_metadata_" + rms_obs_id).append(html);

                  // now send for data
                  url ="/opus/api/metadata/" + rms_obs_id + ".html?cats=" + name;
                  $("#all_metadata_" + rms_obs_id + ' .detail_' + name)
                      .load(url, function() {

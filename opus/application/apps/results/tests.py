...
class resultsTests(TestCase):
    ...
    def test__getFiles(self):
        opus_id = 'S_IMG_CO_ISS_1688230146_N'
        product_types = ['CALIBRATED','RAW_IMAGE']
        files = getFiles(opus_id, fmt='raw', loc_type="path", product_types=product_types)
        print(files[opus_id])
        ...
        print('expected:')
        print(expected[opus_id])
        self.assertEqual(len(files[opus_id]['CALIBRATED']), len(expected[opus_id]['CALIBRATED']))
    ...
    def test__metadata_J_IMG_CO_ISS_1353756055_W(self):
        opus_id = 'J_IMG_CO_ISS_1353756055_W'
        detail, all_info = get_metadata({},opus_id=opus_id,fmt='raw')
        print(detail)
        print(detail['Cassini Mission Constraints'])

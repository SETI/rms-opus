...
- **2026-08-28 (PR-23a, rfrench's ruling):** **a shipped file describes the tree it ships
  with, and never what an earlier commit on `rewrite` held.** `.cursor/rules/doc_python.mdc`
  §2 already says it for prose -- "describe the **current** state of the software only...
  never anchor prose to a moment in time or to migration history" -- and `python.mdc` §4 says
  it for comments, which may not restate modification history. Both had decayed across the
  whole tree, and PR-23a is the sweep. What to know when writing the next one:
  - **The distinction to apply.** A **rule waiver in force** is current state and belongs in
    the developer guide (the public-web-API back-compat one); a **retired** waiver is history
    and belongs in this plan, in these notes and in the commit that retired it, all of which
    a reader can still reach. Same for a dependency that was dropped, a config entry that was
    burned down, a script that was replaced: the fact that survives is the invariant that
    holds now and the reason it holds, and it can always be written without the story.
  - **The shape of the mistake, which is the one to watch for.** Not one of the instances was
    careless. Each was a deliberate attempt to be helpful about a change the same PR had just
    made -- "that is worth stating because it was not always so", "there used to be a second",
    "the first version of this test asserted the end state". The impulse to explain the
    change is exactly the impulse to resist, because the reader of the file has no access to
    the before.
  - **Hand-maintained counts went with them**, per rev 7.17: the bandit skip list's "326 B101
    findings" breakdown and the ruff per-file-ignores table's entry-by-entry burn-down history
    are gone, each replaced by the command that regenerates the measurement.
  - **What was deliberately kept**: statements about a *third party's* history where the
    version is the fact (`VALUES(col)` deprecated in MySQL 8.0.20, the 8.0.19 alias floor);
    user-facing strings about legacy bookmarked URLs and RINGOBS_IDs, which are public-API
    behavior; and pre-existing OPUS comments about the application's own released past. The
    rule is about this branch's history, not about every past tense.
  - **rfrench, on the README**: nobody cares *why* there is a MySQL minimum version. The
...
- **2026-08-31 (PR-24 executed):** `rewrite` merges to `main`. This is the last note; the
  ...
  - **The planning documents are ARCHIVED, not deleted, superseding the plan body.** The
    ...
    sequence as it ran should keep naming the paths that existed while it ran. Outside
    those two directories the old paths were named by `CLAUDE.md`,
    `docs/dev_guide_layout.rst` and `tests/repo/test_pr_references.py`, the last of
    which is deleted here rather than updated.
  ...
  - **The ratchets are finished, and every remaining exception was re-measured rather than
    ...
    `pdstable` 1, `rest_framework` 8); and the `B101` category skip holds back **321**
    findings -- `opus_import` 195, `opus_app` 89, `opus_log_analyzer` 17, `opus_support`
    16, `integration_tests/apps_db_tests` 4, and `opus_config` none -- measured with
    `skips` emptied and `--ignore-nosec`. **Put that override inside `[tool.bandit]`**:
    an earlier measurement here appended it past the end of the table, which bandit
    ignores, and reported 323.
  ...
  - **#1476's seven defects are all fixed, checked against the running server or the
    source, and two of the issue's own claims turned out wrong.** There is no
    `table_order` key in an `api/fields` response -- it is an internal sort key, deleted
    before the response is built -- and the `obs_file` count is four, not five.
    Generalizing the "two JSON examples are invalid" item into a check that parses
    **every** literal block found a third: the `api/files` products example was one
    closing brace short. 25 blocks parse now. The `qtype` fix is the substantive one:
    `any` documented an **or** of its two conditions where `get_range_query` builds an
    **and**, and an or admits ranges that do not overlap at all. The same wrong text was
    also live in `apps/help/faq.yaml`, which is served to users; it is corrected too, and
    `integration_tests/test_api/responses/api_help_faq.html` regenerated through the real
    endpoint -- a one-line fixture diff, verified by re-running the comparison afterwards.
    (The issue itself stays open until this merge closes it.)

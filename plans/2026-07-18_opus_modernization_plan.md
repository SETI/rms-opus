...
- **form_type → alias decision table (exhaustive; no executor judgment).** **Superseded
  ...
  **Two `MultField` value types are corrected against the tree:** `col_val: str | int | None`
  (e.g. `obs_volume_vg2810.py:169` passes an int; the VGISS/GOSSI filter-number fields pass a
  label value PDS carries as a number) and `group_disp_order: str | None` (54 of the 410

...
- **Adversarial pre-PR review (required, before opening the PR):** once the work is
  ...
  **fresh, independent opus-class sub-agent** to adversarially review the full diff
  ...
  - **The orchestrator gates every pass after the first (rev 7.17).** The executor does
    **not** start another pass on its own judgment: it reports each finding in one line with
    its disposition and, crucially, its own **blocking / non-blocking** classification, then
    waits for a go/no-go.
    - **Blocking** — a correctness, security, data-loss or behavior-change defect, or a
      ...
      something different*. Worth a fix and another pass.
    - **Non-blocking** — wording, emphasis, formatting, a stale count, a claim that
      overstates without misleading anyone into acting wrongly. **Fix once, in a single
      batch, and do not spend a pass re-reviewing it.**
    A pass whose findings are all non-blocking **ends the loop**. Prose-only changes never
    earn a re-review. Brief every reviewer to mark cosmetic findings as such rather than
    reporting them at the weight of a defect.
